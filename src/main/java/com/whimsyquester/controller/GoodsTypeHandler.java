 ...
 * @FilePath: \src\main\java\com\whimsyquester\controller\GoodsTypeHandler.java
...
public class GoodsTypeHandler {
    ...
    public void addType(GoodsType type, HttpServletResponse response) throws Exception {
        ...
        if (flag) {
            // 如果添加成功,则使用response.sendRedirect()重定向到"/Ascent/admin/goods_sort.html"页面
            response.sendRedirect("/Ascent/admin/goods_sort.html");
        } else {
            // 否则使用response.setContentType()方法设置响应的MIME类型和字符集编码
            response.setContentType("text/html;charset=utf-8");
            // 然后使用response.getWriter()方法获取输出流,输出一个JavaScript弹窗提示添加失败,并使用location.href属性将页面重定向到"/Ascent/admin/add_type.html"页面
            response.getWriter()
                    .println("<script>alert('添加失败！');location.href='/Ascent/admin/add_type.html';</script>");
        }
    }
    ...
    public void AdminUpdateType(GoodsType type, HttpServletResponse response) throws Exception {
        ...
        if (flag) {
            // 如果更新成功,则使用response()重定向到"/Ascent/admin/goods_sort.html"页面
            response.sendRedirect("/Ascent/admin/goods_sort.html");
        } else {
            // 否则使用response.setContentType()方法设置响应的MIME类型和字符集编码
            response.setContentType("text/html;charset=utf-8");
            // 然后使用response.getWriter()方法获取输出流,输出一个JavaScript弹窗提示修改失败,并使用location.href属性将页面重定向到"/Ascent/admin/goods_sort.html"页面
            response.getWriter()
                    .println("<script>alert('修改失败！');location.href='/Ascent/admin/goods_sort.html';</script>");
        }
    }
}
