 ...
 * @FilePath: \src\main\java\com\whimsyquester\controller\GoodsHandler.java
...
import com.whimsyquester.service.inter.GoodsServiceInter;
...
public class GoodsHandler {
    ...
    public void adminUpdateGoods(Goods good, HttpServletResponse response) throws Exception {
        ...
        if (flag) {
            // 如果修改成功将重定向到商品列表页面
            response.sendRedirect("/Ascent/admin/goods_list.html");
        } else {
            // 否则将弹出一个提示框,提示修改失败
            response.setContentType("text/html;charset=utf-8");
            response.getWriter()
                    .println("<script>alert('修改失败！');location.href='/Ascent/admin/goods_list.html';</script>");
        }
    }
    ...
    public void AdminaddGoods(Goods good, HttpServletResponse response, MultipartFile imgs) throws Exception {
        ...
        if (flag) {
            ...
            response.setContentType("text/html;charset=utf-8");
            response.getWriter()
                    .println("<script>alert('添加失败！');location.href='/Ascent/admin/goods_list.html';</script>");
        }
    }
    ...
    public void adminGoodsBytype(Goods god, HttpServletResponse response) throws Exception {
        ...
        if (flag) {
            ...
            response.sendRedirect("/Ascent/admin/goods_type_update.html");
        } else {
            // 否则将弹一个提示框,提示转移失败
            response.setContentType("text/html;charset=utf-8");
            response.getWriter()
                    .println("<script>alert('转移失败！');location.href='/Ascent/admin/goods_list.html';</script>");
        }
    }
}
