 ...
 * @FilePath: \src\main\java\com\whimsyquester\controller\ShiroHandler.java
...
public class ShiroHandler {
	...
	public String updateAdminShiro(@PathVariable Integer shiroId, @PathVariable Integer userId) throws Exception {
		...
		if (shiroId == null || userId == null) {
			// 如果其中任意一个为null,则返回一个JavaScript脚本,该脚本在客户端弹出一个警告框,然后将页面重定向到"/Ascent/admin/AdminUserShiro.html"路径
			return "<script>alert('非法请求！');location.href='/Ascent/admin/AdminUserShiro.html'</script>";
		} else {
			// 如果"shiroId"和"userId"都不为null,则将它们传递给shiroInter.AdminUpdateShi()方法进行处理
			boolean flag = shiroInter.AdminUpdateShi(shiroId, userId);
			// 如果更新成功,则返回一个JavaScript脚本,该脚本在客户端弹一个提示框,然后将页面重定向到"/Ascent/admin/AdminUserShiro.html"路径
			if (flag) {
				return "<script>alert('修改成功');location.href='/Ascent/admin/AdminUserShiro.html'</script>";
			} else {
				// 如果更新失败,则返回一个JavaScript脚本,该脚本在客户端弹出一个提示框,然后将页面重定向到"/Ascent/admin/AdminUserSh.html"路径
				return "<script>alert('修改失败');location.href='/Ascent/admin/AdminUserShiro.html'</script>";
			}
		}
	}
}
