 ...
 * @FilePath: \src\main\java\com\ssm\controller\OrdersHandler.java
...
public class OrdersHandler {
	...
	public String adminDaiDels(Integer id, Integer shiro_id) throws Exception {
		...
		if (id == null || shiro_id == null) {
			// 如果其中任何一个为null,则返回一个JavaScript脚本,该脚本在客户端弹出一个警告框,然后将页面重定向到"/Shop-SSM/admin/orders_dai.html"路径
	...
	public String adminfahuoOrdes(Integer id) throws Exception {
		// 检查"id"是否为null
		if (id == null) {
			// 如果"id"为null,则返回一个JavaScript脚本,该脚本在客户端弹出一个警告框,然后将页面重定向到"/Shop-SSM/admin/orders_weifa.html"路径
			...
			if (flag) {
				// 则返回一个JavaScript脚本,该脚本在客户端弹出一个提示框,显示"发货成功",然后将页面重定向到"/Shop-SSM/admin/ordersifa.html"路径
